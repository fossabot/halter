...
def build_network(page: ft.Page) -> ft.Card:
    ...
    def on_color_change(color: dict) -> None:
        ...
        for ctrl in [net_name, net_class, ip_addr, mask, vlan, gateway]:
            ...
            if hasattr(ctrl, "border_color"):
                ctrl.border_color = text_color  # если хочешь и бордер перекрасить
    ...
    ip_addr = ft.TextField(
        ...
        width=300,
        adaptive=True,
        on_blur=lambda e: validator.validate(ValidationType.IPv4NetworkAddress, e, mask.value),
    )
    ...
    card_container.content = ft.Column([net_name, net_class, ip_addr, mask, vlan, gateway, color])

    return ft.Card(content=card_container)


def build_network_container(page: ft.Page) -> ft.Container:
    cards: list[ft.Card] = [build_network(page) for _ in range(4)]  # Пример: 10 карточек
